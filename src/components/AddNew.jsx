...
const AddNew = ({ clickFunc, fetchF }) => {
	...
	return (
		<div className="absolute top-0 left-0 h-screen w-full flex items-center justify-center">
			...
			<div className="absolute h-[40vh] w-[30vh] bg-zinc-800 z-20 border rounded-xl p-9 flex flex-col items-center justify-center">
				<div className="relative w-full my-3 py-1 text-blue-400">
					...
					<label
						htmlFor="quote"
						className={`absolute left-0 top-1 cursor-text peer-focus:text-xs peer-focus:text-orange-600/70 dark:peer-focus:text-orange-300/70 peer-focus:-top-4 transition-all duration-300  ${
							value != "" && " -top-[15px] text-xs text-blue-500"
						}`}>
						Enter Quote:
					</label>
				</div>
				...
				{messageVisible && (
					<div className="text-green-400">Successfully Added New Quote! </div>
				)}
			</div>
		</div>
	);
